...
data <- datagen(y="con",N = 2000, k = 20, random_d = T, theta = "con_lin", var = 1,iter=1) # Setting 1
data <- datagen(y="con",N = 2000, k = 20, random_d = "interaction", theta = "con_non", var = 1,iter=1) # Setting 2
k <- ncol(data)-3
...
cate_DR_oob <- bootCI(pred_B=results_cate_DR,score=score_dr_oob)
cate_R_oob <- bootCI(pred_B=results_cate_R,score=score_R_oob)
cate_DR <- bootCI(pred_B=results_cate_DR,score=score_dr)
cate_R <- bootCI(pred_B=results_cate_R,score=score_R)
...
prep_plot <- function(cate){
  cate1 <- reshape2::melt(cate[order(cate$pred),])
...
mm_DR_oob <- prep_plot(cate_DR_oob)
mm_R_oob <- prep_plot(cate_R_oob)
...
mm <- rbind(mm_DR,mm_R,mm_T,mm_X,mm_CBART,mm_CF,mm_DR_oob,mm_R_oob)
mm$Method <- rep(c("DR-learner","R-learner","T-learner","X-learner","Causal-BART","Causal-Forest","DR-learner_oob","R-learner_oob"),each=nrow(mm_DR))
...
mm$Method <- rep(c("DR","R","T-learner","X-learner","C-BART","C-Forest","DR oob","R oob"),each=nrow(mm_DR))
ggplot(mm[mm$variable=="pred",], aes(y=value,x=Method))+
  geom_boxplot() +
  theme_cowplot()+
  geom_hline(yintercept=mean(data$theta),color="blue")
...
mm_DR_oob <- prep_noorder(cate_DR_oob)
mm_R_oob <- prep_noorder(cate_R_oob)
...
mean((cate_DR$pred-data$theta)^2)
mean((cate_R$pred-data$theta)^2)
mean((cate_T$pred-data$theta)^2)
mean((cate_X$pred-data$theta)^2)
mean((cate_CBART$pred-data$theta)^2)
mean((cate_CForest$pred-data$theta)^2)

mean((cate_DR_oob$pred-data$theta)^2)
mean((cate_R_oob$pred-data$theta)^2)
